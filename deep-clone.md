...
Symbols cannot be safely serialized since creating one guarantees a unique value. For example, `Symbol("foo") === Symbol("foo")` is `false`. This means that if you could save an object with a symbol in the IndexedDB API, close the JavaScript runtime so the symbol registry is emptied, and then start the runtime again and load the object, the returned object will contain a symbol with a different value. This is not a safe serialization.
...
The functions returned by `getIterator` still access data in their closure, but that closure in contained within the factory. Every call of `getIterator` creates a new closure, so the functions can be safely cloned. If we use the "registry" technique showed in the repeater example, then iterators could also be cloned.
